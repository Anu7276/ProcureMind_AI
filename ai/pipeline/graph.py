...
The compiled graph is a callable that accepts a PipelineState dict and
returns the updated state after all nodes have run.
"""
...
async def run_pipeline(
    ...
    """
    Entry point for the full 5-node pipeline.
...
async def run_pipeline_from_requirement(
    ...
    Used when /recommend receives output from a prior /ingest call.
    """
    if audit_id is None:
        audit_id = str(uuid.uuid4())

    # We create a partial state that skips Nodes 00-02 by pre-populating
    # their output fields. The graph still runs all nodes but 00-02 become
    # near-no-ops since their inputs are already resolved.
    initial_state: PipelineState = {
